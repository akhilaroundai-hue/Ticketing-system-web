-- Run this script manually in your Supabase SQL editor to create the voice_notes storage bucket
...
DROP POLICY IF EXISTS "Allow authenticated users to upload voice notes" ON storage.objects;
...
DROP POLICY IF EXISTS "Allow authenticated users to read voice notes" ON storage.objects;
...
DROP POLICY IF EXISTS "Allow agents to update voice notes" ON storage.objects;
...
DROP POLICY IF EXISTS "Allow agents to delete voice notes" ON storage.objects;
...
SELECT * FROM storage.buckets WHERE id = 'voice_notes';
